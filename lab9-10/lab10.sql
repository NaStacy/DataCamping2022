...
FROM
  ( 
    SELECT 
      TO_DATE( '12/31/500', 'MM/DD/YYYY' ) sd,
      rownum rn
    FROM dual
...
EXPLAIN PLAN FOR
SELECT /*+ parallel(TIMES, 4)*/* FROM calendar 
WHERE day_name = '—Û··ÓÚ‡';
...
WHERE calendar_week_number IN(2, 1);

select plan_table_output from table(dbms_xplan.display());

EXPLAIN PLAN FOR
DELETE /*+ parallel(TIMES, 4)*/ FROM calendar
WHERE calendar_week_number IN(2, 1);
...
drop table calendar;
EXPLAIN PLAN FOR 
CREATE TABLE calendar parallel 8 AS (SELECT * FROM
(SELECT 
  TRUNC( sd + rn ) time_id,
  TO_CHAR( sd + rn, 'fmDay' ) day_name,
  TO_CHAR( sd + rn, 'D' ) day_number_in_week,
